...
class ReparametrizeWrapper(ObservationRewardWrapper):
    ...
    def step(self, action):
        observation, reward, terminated, truncated, info = self.env.step(action)
        return self.observation(observation), self.reward(reward,truncated), terminated, truncated, info
...
class PILCOWrapper(ObservationRewardWrapper):
    ...
    def step(self, action):
        observation, reward, terminated, truncated, info = self.env.step(action)
        return self.observation(observation), self.reward(observation), terminated, truncated, info
